...
use totils::trace_collector::TRACE_ONLY;
use tracing::info;
...
async fn main() -> miette::Result<ExitCode> {
    let (trace_collector_layer, mut trace_rx) = totils::trace_collector::TraceCollector::new(1024);

    spawn(async move {
        while let Some(trace) = trace_rx.recv().await {
            // in this example we just print the trace, in a real application you'd probably want to do something more useful with it, like feeding it into your observability pipeline
            eprintln!("Received trace: {}", trace.value);
        }
    });
    ...
    tracing_subscriber::registry()
        ...
        .with(trace_collector_layer.with_filter(TRACE_ONLY))
        .init();
